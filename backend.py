...
# Rate limiting için basit bir yapı
class RateLimiter:
...
rate_limiter = RateLimiter(max_requests=50, per_seconds=3600)  # 50 istek/saat/IP

# Ana HTML içeriği (önceki kodu buraya kopyalayacağız)
        ...
        return render_template_string('''
            ...
            <head>
                ...
                <style>
                    ...
                    .warning { background: #ffebee; border: 2px solid #f44336; padding: 30px; border-radius: 10px; max-width: 600px; margin: 0 auto; }
                </style>
            </head>
            <body>
                <div class="warning">
                    ...
                    <p>IP: {{ ip }}</p>
                    <p>Bu bir güvenlik önlemidir. Normal kullanımda bu limit aşılmaz.</p>
                    <a href="/" style="margin-top: 20px; display: inline-block; padding: 10px 20px; background: #0088cc; color: white; text-decoration: none; border-radius: 5px;">Ana Sayfaya Dön</a>
                </div>
            </body>
            </html>
        ''', ip=client_ip), 429
...
def check_rate_limit():
    """API endpoint for rate limit check"""
    client_ip = get_client_ip()
...
Allow: /
Disallow: /admin/
Sitemap: https://panel.art/sitemap.xml'''
